...
def predict_tavg(model_tavg, scaler_X_tavg, scaler_y_tavg, data, steps, last_date, forecast=None):
    input_features = [
        'TAVG',
        'sin_day',
        'cos_day',
    ...
    current_input = scaler_X_tavg.transform(data.values)

    future_preds_scaled = []

    for i in range(steps):
        pred_scaled = model_tavg.predict(
            current_input.reshape(1, current_input.shape[0], current_input.shape[1]),
            verbose=0
        )[0][0]

        pred_actual = scaler_y_tavg.inverse_transform(
            [[pred_scaled]]
        )[0][0]
        ...
        pred_scaled_var = scaler_y_tavg.transform(
            [[pred_actual_var]]
        )[0][0]
    ...
    future_preds = scaler_y_tavg.inverse_transform(
        np.array(future_preds_scaled).reshape(-1, 1)
    ).flatten()

    future_preds = np.round(future_preds, 2)

    # return round(float(future_preds[-1]), 2)
